...
export default function TrackingPage() {
  ...
  const handleStatusChange = (id: number, newStatus: string) => {
    ...
    const updated = allCompanies.map((c: any) => 
  ...
  const filteredCompanies = useMemo(() => {
    ...
    // ADDED: Filter by marketer
    if (filterMarketer !== "all") {
      result = result.filter((c) => c.registered_by === filterMarketer);
    }

    return result;
  }, [companies, searchTerm, filterStatus, filterPlan, filterMarketer]); // CHANGED: Added filterMarketer to dependencies
  ...
  const stats = useMemo(() => {
    ...
    return { total, active, today, thisWeek };
  }, [companies]);
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-secondary/60">
              Monitor and manage all registered companies
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm hover:shadow-md transition">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-gray-500">Registered Today</span>
      ...
      <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {/* ADDED: Marketer filter dropdown */}
            <select
            ...
            >
              ...
              {/* Get unique marketers from data */}
              {[...new Set(companies.map(c => c.registered_by))].filter(Boolean).map((marketer) => (
                <option key={marketer} value={marketer}>{marketer}</option>
              ))}
            </select>
            {(searchTerm || filterStatus !== "all" || filterPlan !== "all" || filterMarketer !== "all") && (
              <button
                onClick={() => {
                  setSearchTerm("");
                  setFilterStatus("all");
                  setFilterPlan("all");
                  setFilterMarketer("all"); // ADDED: Reset marketer filter
                }}
...
<th className="p-4 text-xs font-semibold text-secondary uppercase tracking-wider">Company Name</th>
...
<th className="p-4 text-xs font-semibold text-secondary uppercase tracking-wider hidden sm:table-cell">Marketer</th> 
                ...
                <tr>
                  <td colSpan={9} className="p-12 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-sm text-gray-400 mt-1">Register a company to start tracking</p>
                    </div>
                  </td>
                </tr>
              ) : (
  ...
  <div className="flex items-center gap-2">
    <div className="w-6 h-6 rounded-full bg-purple-100 flex items-center justify-center">
      <span className="text-xs font-bold text-purple-600">
        {company.registered_by?.[0] || "?"}
      </span>
    </div>
    <span className="text-sm font-medium text-gray-700">
      {company.registered_by || "Unknown"}
    </span>
  </div>
</td>  {/* ADDED: Marketer column */}
<td className="p-4">
...
<td className="p-4 hidden sm:table-cell">
  <select
    ...
    className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border-0 focus:ring-2 focus:ring-secondary/20 ${getStatusColor(company.subscription_status)}`}
  >
    <option value="Active">Active</option>
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium mt-1 ${getStatusColor(selectedCompany.subscription_status)}`}>
