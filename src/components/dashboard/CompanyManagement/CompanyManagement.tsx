  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingSlug && originalFormData) {
      const hasChanges = () => {
        ...
        // Check file uploads - only detect actual user actions
        // For logo: 
        // - True if user uploaded a new file (formData.logo is File)
        // - True if user removed existing image (formData.logo === null AND originalFormData.logo is a string URL)
        // - False if formData.logo is null and originalFormData.logo is null (no image ever)
        // - False if both have URL strings (means image unchanged - should not happen as formData.logo is always null or File)
        const logoChanged = 
          (formData.logo instanceof File) || 
          (formData.logo === null && originalFormData?.logo !== null && typeof originalFormData.logo === 'string');
        const coverChanged = 
          (formData.cover_image instanceof File) || 
          (formData.cover_image === null && originalFormData?.cover_image !== null && typeof originalFormData.cover_image === 'string');
