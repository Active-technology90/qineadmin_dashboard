...
const CopyButton = memo(({ text }: { text?: string | number | null }) => {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.95 }}
      onClick={handleCopy}
      className="text-gray-400 hover:text-[#674FA3] transition-colors p-1 rounded-md bg-white/50 backdrop-blur-sm"
  ...
  >
    <div className="flex flex-row justify-between items-start">
      <div className="flex items-center gap-2 mb-4 pb-3 w-full relative">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-[#674FA3] via-[#8B6BB5] to-transparent rounded-full" />
      </div>
    ...
    >
      ...
      <div className="px-2 sm:px-5 md:px-6 py-2 sm:py-4 border-b border-gray-100 bg-gray-50/30 flex flex-wrap items-start sm:items-center justify-between gap-1 sm:gap-3 w-full">
        <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
          <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white rounded-lg sm:rounded-xl flex items-center justify-center shadow-sm border border-gray-100 flex-shrink-0">
            {vendorOrder.company?.logo ? (
              ...
              <Store className="h-4 w-4 sm:h-5 sm:w-5 text-[#674FA3]" />
            )}
          </div>
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 sm:gap-3">
          <span
            className={`px-1 sm:px-2.5 py-0.5 sm:py-1 text-[8px] sm:text-xs font-semibold rounded-full border ${getStatusBadge(vendorOrder.status)}`}
          >
          ...
          <div className="text-right flex-shrink-0">
            <span className="text-[11px] sm:text-lg font-bold text-[#674FA3] tracking-tight block">
      ...
      {vendorOrder.delivery && (
        <div className="bg-gradient-to-r from-purple-50/40 to-transparent px-3 sm:px-5 md:px-6 py-2 sm:py-3 border-b border-purple-100/50 w-full">
          <div className="flex items-center gap-2 text-xs font-semibold text-[#674FA3] mb-2">
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-4 text-[10px] sm:text-sm">
            ...
            {vendorOrder.delivery.tracking_id && (
              <div className="flex items-center gap-1 sm:gap-2 bg-white/50 rounded-full px-1.5 sm:px-3 py-0.5 sm:py-1">
                ...
                <CopyButton
                  text={vendorOrder.delivery.tracking_id.split("-")[0]}
                />
              </div>
            )}
        ...
        <div className="px-2 sm:px-5 md:px-6 py-3 sm:py-6 w-full">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-5 gap-2">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
              <div className="flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-xl sm:rounded-2xl border border-[#674FA3]/15 bg-gradient-to-br from-[#674FA3]/10 to-violet-100 shadow-sm flex-shrink-0">
                <Package className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-[#674FA3]" />
              </div>
            ...
            <div className="inline-flex items-center gap-2 self-start rounded-xl sm:rounded-2xl border border-[#674FA3]/10 bg-[#674FA3]/5 px-2 sm:px-3 py-1.5 sm:py-2 flex-shrink-0">
              <div className="h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full bg-[#674FA3] animate-pulse" />

              <span className="text-[9px] sm:text-[11px] font-bold tracking-wide text-[#674FA3] whitespace-nowrap">
                Vendor Products
              </span>
            </div>
          </div>
          ...
          <div className="overflow-hidden rounded-[26px] border border-gray-200/80 bg-white shadow-[0_10px_35px_rgba(0,0,0,0.04)]">
            ...
            "
                >
                  ...
                  "
                      >
                        <span className="text-[8px] sm:text-sm font-black text-[#674FA3]">
                          ×{item.qty}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="sm:col-span-2 flex flex-row sm:flex-col items-center justify-between sm:justify-end mt-1 sm:mt-0">
                    <div className="flex flex-row sm:flex-col items-center gap-1 sm:gap-0 w-full sm:text-right">
                      ...
                      <div>
                        ...
                        <p className="text-[7px] sm:text-[10px] font-bold uppercase tracking-[0.18em] text-[#674FA3]">
                          ETB
                        </p>
                      </div>
                    </div>
...
const FinancialCard = memo(({ order }: { order: MasterOrder }) => (
  <motion.div
    ...
    className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#674FA3] via-[#5a448c] to-[#4a3a78] p-4 sm:p-6 shadow-lg w-full"
  >
    ...
    <div className="relative z-10">
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="text-right">
          <p className="text-[8px] sm:text-[10px] font-black uppercase tracking-[0.2em] text-white/50">
            Total
          </p>
          <h3 className="text-lg sm:text-2xl font-black text-white">
            {Number(order.total_amount).toLocaleString()}{" "}
            <span className="text-[10px] sm:text-sm">ETB</span>
          </h3>
        </div>
      </div>
      <div className="space-y-2 sm:space-y-3 pt-2 border-t border-white/10">
        ...
        {order.vendor_orders?.map((vo) => (
          <div
            key={vo.id}
            className="flex justify-between text-[10px] sm:text-xs font-medium text-white/70 pl-2 sm:pl-3 border-l border-white/20"
          >
            <span className="opacity-80 truncate max-w-[100px] sm:max-w-none">
              {vo.company?.name || "Vendor"} Delivery
            </span>
            <span className="flex-shrink-0">
              {Number(vo.delivery_fee || 0).toLocaleString()} ETB
            </span>
          </div>
        ))}
...
const TimelineCard = memo(({ order }: { order: MasterOrder }) => (
  ...
  >
    <div className="flex items-center gap-2 mb-6">
      <Calendar className="h-4 w-4 text-[#674FA3]" />
      ...
      <div className="relative flex gap-4">
        <div className="flex flex-col items-center">
          <div className="relative">
            <div className="w-2.5 h-2.5 rounded-full bg-[#674FA3] ring-4 ring-[#674FA3]/10" />
...
const CustomerCard = memo(({ order }: { order: MasterOrder }) => (
  <Card title="Customer Profile" icon={User}>
    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-3 sm:gap-4">
      <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-br from-purple-100 to-purple-200 border-2 border-white shadow-sm flex items-center justify-center text-[#674FA3] font-black text-lg sm:text-xl flex-shrink-0">
        {getInitials(order.recipient_name || "?")}
      </div>
      <div className="space-y-1 text-center sm:text-left">
        <p className="font-black bg-gradient-to-r from-[#674FA3] to-[#8B6BB5] bg-clip-text text-transparent break-words">
...
const ShippingCard = memo(({ order }: { order: MasterOrder }) => (
  <Card title="Shipping Destination" icon={MapPin}>
    <div className="space-y-3">
      <div className="flex flex-col sm:flex-row items-start gap-1 sm:gap-2 p-2 rounded-lg bg-purple-50/30 border-l-4 border-[#674FA3]">
        <span className="text-[10px] sm:text-xs font-bold text-gray-500 sm:min-w-[80px]">
          Recipient:
        </span>
        <span className="text-xs sm:text-sm font-bold text-[#674FA3] break-words">
          {order.recipient_name || "N/A"}
        </span>
      </div>
      <div className="flex flex-col sm:flex-row items-start gap-1 sm:gap-2 p-2 rounded-lg bg-blue-50/30 border-l-4 border-green-500">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <PhoneCall className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-green-600 flex-shrink-0" />
          <span className="text-[10px] sm:text-sm font-mono font-bold text-green-700 break-all">
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6">
        ...
        >
          ...
          <div className="bg-[#674FA3]/20 backdrop-blur-md border-b border-[#674FA3]/10 px-3 sm:px-6 md:px-8 py-2 sm:py-4 md:py-5 sticky top-0 z-20 shadow-sm">
                ...
                <div className="col-span-12 lg:col-span-8 space-y-4 sm:space-y-6 md:space-y-8 min-w-0">
                  ...
                  {order.vendor_orders && order.vendor_orders.length > 0 ? (
                    <div className="space-y-4 sm:space-y-6 min-w-0">
                      <div className="flex items-center gap-2">
                        <Building2 className="h-5 w-5 text-[#674FA3]" />
