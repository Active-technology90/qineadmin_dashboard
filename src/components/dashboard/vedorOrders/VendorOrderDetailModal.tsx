...
const DeliveryCard = ({
        ...
        if (entry && typeof entry === 'object') {
          if (entry.driver_id === driverId || entry.id === driverId) {
            return {
              ...
              lon: entry.lon ?? entry.longitude ?? entry.lng,
            };
          }
        }
      }
      const directMatch = (liveDriverLocations as any)[driverId] || 
                         (liveDriverLocations as any)[String(driverId)];
      if (directMatch) {
        return {
          lat: directMatch.lat ?? directMatch.latitude,
          lon: directMatch.lon ?? directMatch.longitude ?? directMatch.lng,
        };
      }
    }
    return null;
  ...
  const staffWithDistance = useMemo(() => {
    ...
    return staffList.map(s => {
      if (!customerLoc) {
        return { ...s, calculated_distance: null, location_source: null };
      }
      ...
      if (liveLoc && isValidCoordinate(liveLoc.lat, liveLoc.lon)) {
        ...
        return { ...s, calculated_distance: distance, location_source: 'live' };
      }
      if (s.current_lat != null && s.current_lng != null) {
        ...
        if (isValidCoordinate(currentLat, currentLng)) {
          ...
          return { ...s, calculated_distance: distance, location_source: 'current' };
        }
      }
      if (s.last_lat != null && s.last_lon != null) {
        ...
        if (isValidCoordinate(lastLat, lastLon)) {
          ...
          return { ...s, calculated_distance: distance, location_source: 'last_known' };
        }
      }
      if (s.distance_km != null && !isNaN(parseFloat(s.distance_km))) {
        return { ...s, calculated_distance: parseFloat(s.distance_km), location_source: 'api' };
      }
      return { ...s, calculated_distance: null, location_source: null };
    });
  }, [staffList, liveDriverLocations, delivery?.customer_lat, delivery?.customer_lon, order?.customer_lat, order?.customer_lon, order?.delivery_address?.lat, order?.delivery_address?.lon]);
  ...
  const filteredStaffList = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      if (a.calculated_distance == null && b.calculated_distance == null) return 0;
      ...
      if (filter === "all") {
        try {
          const [inHouseRes, thirdPartyRes] = await Promise.all([
            getAvailableDeliveryDrivers(order.company.slug, true),
            getAvailableDeliveryDrivers(order.company.slug, false),
          ]);
          allDrivers = [
            ...(inHouseRes.data || []),
            ...(thirdPartyRes.data || []),
        ...
        try {
          const res = await getAvailableDeliveryDrivers(order.company.slug, true);
          allDrivers = res.data || [];
        } catch (apiError) {
          console.warn("API fetch failed, using mock data", apiError);
          allDrivers = mockDrivers.filter(d => d.is_in_house);
        }
      } else {
        try {
          const res = await getAvailableDeliveryDrivers(order.company.slug, false);
          allDrivers = res.data || [];
        } catch (apiError) {
          console.warn("API fetch failed, using mock data", apiError);
          allDrivers = mockDrivers.filter(d => !d.is_in_house);
