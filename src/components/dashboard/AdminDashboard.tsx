...
export default function AdminDashboard() {
  ...
  // Determine user role based on memberships (supports multiple memberships)
  ...
  const isCompanyUser = hasMembership; // user has at least one company
    ...
    <div className="flex h-screen font-sans bg-gray-50 overflow-hidden">
      ...
      >
        ...
        <nav className="flex-1 px-4 py-6 space-y-1.5 overflow-y-auto custom-scrollbar">
          ...
          {/* Platform Admin section – only for super admin (no memberships) */}
          {!isCompanyUser && (
            <>
          ...
          {/* Companies – always visible (company users see only their own company) */}
          <button
            onClick={() => {
          ...
          {/* Company Products – always visible */}
          <button
            onClick={() => {
          ...
          {/* Company Users – visible for super admin AND company admin (not staff) */}
          {!isCompanyStaff && (
            <button
              onClick={() => {
                setActiveTab("users");
